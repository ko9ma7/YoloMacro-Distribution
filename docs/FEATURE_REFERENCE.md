...
- 마우스, 키보드, 드래그, 휠, 랜덤 좌표
- 마우스/키 입력과 독립된 `찾은 동일 대상을 계속 따라가기`
- 점진적 이동·회전·투명도 변화는 OpenCV 동일성 추적, 큰 외형 변화·재등장은 YOLO 보정 추적
- 추적 주기·최대 시간·유실 허용·검색 반경·YOLO 재탐지 주기 설정
- 비활성 입력, 실제 커서, Interception, DLL 인젝션
- 웹훅 메시지
- 동작 전·후 딜레이

동일 대상 추적의 사용법과 한계는 [동일 대상 추적 가이드](TARGET_IDENTITY_TRACKING_GUIDE.md)를 참고하세요.
