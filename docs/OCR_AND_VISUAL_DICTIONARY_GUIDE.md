...
YoloMacro 1.4.5는 범용 Tesseract OCR, 대상 프로그램 전용 글자 샘플 OCR, 라벨별 대표 이미지를 모아 상태를 판정하는 `대표 이미지 사전`을 제공합니다. 모두 대상 창과 설정에서 선택한 카메라 프레임, ROI, 기존 발견/미발견 분기, 클릭·키 입력, Replay·능동 학습 흐름을 그대로 사용합니다.
...
2. 최초에 클릭할지 여부를 `마우스 액션`에서 별도로 선택합니다.
3. `찾은 동일 대상을 계속 따라가기`를 켭니다.
4. 점진적인 이동·회전·투명화에는 `시각 동일성`, 큰 외형 변화나 가림 뒤 재탐지에는 `하이브리드`를 선택합니다.
5. 추적 간격은 일반적으로 20~50ms, 재탐색 반경은 프레임 사이 예상 이동 거리보다 크게 설정합니다.
6. `최대 시간 0`은 대상이 연속 이탈 프레임 수만큼 사라지거나 사용자가 정지할 때까지 계속합니다.

완전히 투명해져 배경과 같은 픽셀이 된 대상은 어떤 비전 모델도 직접 볼 수 없습니다. 이때는 최근 이동 속도로 짧게 예측하고 이탈 허용을 넘으면 종료합니다. 자세한 사용법은 [동일 대상 추적 가이드](TARGET_IDENTITY_TRACKING_GUIDE.md)를 참고하세요.

![대상 추적 설정](../artifacts/action-settings-target-follow-v1.4.5.png)
