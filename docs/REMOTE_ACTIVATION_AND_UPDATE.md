...
> v1.4.4 배포 주의: OCR은 관리 DLL, 네이티브 런타임과 언어 데이터를 함께 사용합니다. 기존 CoreLib 단일 파일 updater는 이 의존성을 설치할 수 없으므로 v1.4.4는 전체 ZIP을 새 폴더에 풀어 업그레이드합니다. 화면의 `설치 버전`은 실행 파일 버전이고 `자동 DLL 정책`은 이 구형 updater의 마지막 호환 버전입니다.
...
프로그램의 `GitHub` 버튼은 토큰이나 비밀번호를 프로그램에 저장하지 않고 기본 브라우저로 공개 배포 저장소 `ko9ma7/YoloMacro-Distribution`을 엽니다. GitHub 로그인 여부와 설비 활성화 여부는 서로 독립적이며, 활성화 판정은 아래의 서명된 manifest만 사용합니다.
...
```json
{
  ...
  "updateUrl": "https://github.com/ko9ma7/YoloMacro-Distribution/releases/download/v1.4.2/YoloMacro.CoreLib.dll",
  "releaseNotesUrl": "https://github.com/ko9ma7/YoloMacro-Distribution/releases/tag/v1.4.2",
  "dllVersion": "1.4.2",
