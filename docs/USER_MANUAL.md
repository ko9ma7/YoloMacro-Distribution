...
이 문서는 YoloMacro를 처음 실행하는 단계부터 화면 인식 자동화, OCR, 대표 이미지 사전, AOI 검사, YOLO 데이터 제작과 모델 연결까지 전체 흐름을 안내하는 문서의 시작점입니다. 현재 정식 프로그램 버전은 `1.4.3`입니다.
...
- 상위 작업은 화면에 보이는 순서대로 검사합니다. 조건 폴더가 발견되면 그 폴더의 체크된 하위 작업을 위에서 아래로 수행한 뒤 다음 상위 작업으로 진행합니다.
- `액션 후 화면 갱신`이 켜져 있으면 클릭이나 키 입력 직후 기존 캡처를 버리고 새 화면을 캡처한 다음 항목을 검사합니다. 목록 끝에서만 0번 항목으로 돌아갑니다.
- 액션의 `동작 대기`는 `전 대기 → 마우스/키 입력 → 후 대기 → 다음 항목` 순서입니다. 시작 로그와 각 동작 로그에서 실제 값을 확인할 수 있습니다.
- 한 번 찾은 위치를 반복 클릭하려면 `고속 반복 좌클릭`을 고르고 횟수와 간격을 설정합니다. 횟수는 1~10,000회, 간격은 0~1,000ms이며 F6으로 중단할 수 있습니다.
- 키 감지 결과는 숫자 `1`, Space `{SPACE}`, Enter `{ENTER}`처럼 저장됩니다. 게임이나 앱플레이어가 비활성 입력을 무시하면 `입력 우회`에서 실제 마우스·키보드 또는 Interception을 선택합니다.
- `▶`와 `📁`는 실행 상태와 폴더를 보여주는 화면 표시일 뿐 작업 이름에 저장되지 않습니다. 이전 파일에 남은 반복 접두사는 불러올 때 자동으로 정리됩니다.
...
YoloMacro 내부 기능 설명은 이 저장소의 `v1.4.3` 코드와 실제 UI를 기준으로 작성했습니다. YOLO 학습·ONNX 변환 명령은 최신 Ultralytics 공식 문서를 기준으로 하며, 설치된 Ultralytics 버전에 따라 기본 모델 이름이나 옵션이 달라질 수 있습니다.
