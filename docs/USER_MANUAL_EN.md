...
1. Download `YoloMacro-v1.4.5-win-x64.zip` from the [public v1.4.5 release](https://github.com/ko9ma7/YoloMacro-Distribution/releases/tag/v1.4.5).
...
See the [OCR and Visual Dictionary Guide](OCR_AND_VISUAL_DICTIONARY_GUIDE_EN.md) for general/custom/hybrid OCR, glyph samples and regex/runtime variables. Use the [Target Identity Tracking Guide](TARGET_IDENTITY_TRACKING_GUIDE_EN.md) for separate click/tracking controls, fading targets and YOLO reacquisition. A v1.4.5 installation must be extracted as a complete ZIP because OCR requires the included managed/native Tesseract files and language data.
...
- Top-level items are evaluated in visible order. When a folder gate matches, enabled child actions run from top to bottom before execution continues with the next top-level item.
- With **Refresh screen after action** enabled, an input discards the old frame, captures the updated screen, and continues with the next item. Only reaching the end of the list restarts at item zero.
- Each action runs as `pre-delay → mouse/key input → post-delay → next item`. The runtime log reports these values and the next item.
- **Rapid Click** reuses one detected position for 1 to 10,000 left clicks with a 0 to 1,000 ms interval. F6 can cancel it.
- Captured keys use visible SendKeys syntax: `1`, `{SPACE}`, `{ENTER}`, or modifiers such as `^c`. Inactive delivery sends key-down and key-up messages and lets Windows translate printable characters without duplicating them; use real keyboard/mouse or Interception only for applications that ignore background messages.
- `▶` and `📁` are display-only markers. They are never persisted in action names, and old repeated markers are normalized when a project is loaded.
