...
이번 재빌드는 Compact 설정에서 선택한 색상 무시 프리셋이 실제 실행 플래그에 반영되지 않을 수 있던 원인을 수정하고, [색상 불변 이미지 매칭과 Mini 실행](IMAGE_MATCHING_MINI_RUN_2.6.0.md)을 추가했습니다.
...
- 실행 상태·현재 항목·경과 시간을 공유하는 항상 위 Mini 실행창
- 원자 입력을 마친 안전 경계에서 대기하고 같은 문맥에서 재개하는 일시정지
- 목적 기반 `새 작업 만들기`와 정확한 설정 탭으로 연결되는 기능 검색
- 자동 저장 복구본과 원본 프로젝트 저장을 분리하고 프로젝트별 이미지를 격리
- 같은 밀리초의 연속 저장도 덮어쓰지 않고 프로젝트별 최신 백업 40개 보존

## OpenCV 5 이미지 매칭

- B/G/R 각 채널의 Scharr 윤곽을 합쳐 테마·호버·눌림처럼 색조와 밝기가 바뀐 같은 구조를 찾습니다.
- 기본 색상, 정밀 픽셀, 색상 무시, 모양만, 이진화, 크기·회전 프리셋을 공통 정책으로 실행합니다.
- 예전 `Exact + 개별 플래그` 데이터는 옵션을 지우지 않고 `Custom`으로 이관합니다.
- 전체 설정, Compact 설정, 선택 값 일괄 변경, 설정 복사와 ImageMax 가져오기가 같은 런타임 옵션을 저장합니다.
...
- 보안·업데이트 시나리오 18/18
- 대상 동일성 추적·이미지 매칭 시나리오 14/14 (회전·스케일, 색상 불변 구조 및 오탐 거부 포함)
- 일시정지 중 액션 고정·재개·중지, 연속 45회 저장 후 최신 백업 40개 보존 회귀 통과
